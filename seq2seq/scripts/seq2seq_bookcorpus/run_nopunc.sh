...
DATA_PATH="/data"
MODEL_PATH="/data/models/books_nopunc_v2"
...
VOCAB_SOURCE=${DATA_PATH}/vocab.tok.nopunc.txt
VOCAB_TARGET=${DATA_PATH}/vocab.tok.nopunc.txt

TRAIN_SOURCES=${DATA_PATH}/train_input_nopunc.txt
TRAIN_TARGETS=${DATA_PATH}/train_output_nopunc.txt
...
if [ "$op" = "train" ]; then
  ...
  python "$SEQ2SEQ_PATH/bin/train.py"  \
    --config_paths="$CONFIG_PATH/model_nopunc.yaml" \
    --model_params "
