...
tf.app.flags.DEFINE_boolean("bleu", False,
                            "Set to True for reporting bleu score on validation set.")
...
def train():
  ...
  (from_train, to_train, from_dev, to_dev) = get_prepared_data()
...
def compute_bleu():
  ...
  dev_buckets_scale = [sum(dev_bucket_sizes[:i + 1]) / dev_total_size
                         for i in xrange(len(dev_bucket_sizes))]
  with tf.Session() as sess:
    (model, in_vocab, out_vocab) = init_decode(sess)
    ...
    while sentence:
      output = decode_sentence(sess, model, in_vocab, out_vocab, sentence)
  ...
  with tf.Session() as sess:
    ...
    for bucket_id = xrange(len(_buckets)):
      dev_set_bucket = dev_set[bucket_id];

      for pair in dev_set_bucket:
        question = pair[0]
		true_answer = pair[1]
        pred_answer = decode_sentence(sess, model, in_vocab, out_vocab, question)
...
def decode_sentence(sess, model, in_vocab, out_vocab, sentence):
  ...
  return " ".join([tf.compat.as_str(out_vocab[output]) for output in outputs])  
...
def init_decode(sess):
    ...
    return (model, in_vocab, rev_out_vocab)
